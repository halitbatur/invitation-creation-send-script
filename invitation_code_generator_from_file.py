...
def generate_invitation_link(email):
    ...
    try:
        ...
        invitation_link = response_content.get('data', {}).get('invitationLink')
...
def read_emails_from_file(filename):
    """Read emails from a text file, one email per line."""
    try:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python invitation_code_generator_from_file.py <email_file>")
        print("Creating an example email file 'emails.txt'...")
        # Create an example file
        with open("emails.txt", "w") as f:
            f.write("example1@example.com\n")
            f.write("example2@example.com\n")
            f.write("example3@example.com\n")
        print("Example file created. Now run: python invitation_code_generator_from_file.py emails.txt")
        sys.exit(1)
